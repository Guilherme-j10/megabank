...
import DotsComponent from '../../dots/index';

import api from '../../../../config/api';

export default function SideBarDois(){
    const [dadosUser, setDadosUser] = useState({});

    async function loaddados(){
        const dados = await api.get('/getFuldata', {
            headers: {token: localStorage.getItem('tokenMegaBank')}
        });

        setDadosUser({
            firstName: dados.data.nome.split(' ')[0],
            nome: dados.data.nome,
            email: dados.data.email,
            senha: dados.data.senha,
            cargo: dados.data.cargo,
            profileImg: dados.data.pathname
        });
    }
        ...
        <div className="right">
            <div className="where">
                <ul>
                    ...
                    <li>
                        <Link>Notificações <span className="number">8</span></Link>
                    </li>
                </ul>
            </div>
            <header className="headerSideRight">
                <h1>Bem-vindo(a) {dadosUser.firstName}</h1>
            </header>
            <div className="PerfilLine">
                <span className="ProfilePhoto" style={{backgroundImage: `url(${dadosUser.profileImg})`}}></span>
                <div className="infos">
                    <div className="Standard">
                        <h1>{ dadosUser.nome }</h1>
                        <p>{ dadosUser.cargo }</p>
                        <Link to="/account">Editar conta</Link>
                    </div>
                </div>
            </div>
            <div className="moneyShow">
                <div className="mesmaCoisa">
                    <p>Ganhos Totais: </p>
                    <h1>M$ 145</h1>
                </div>
                <div className="mesmaCoisa">
